...
namespace JobObjectsPOC
{
    internal class Program
    {
        ...
        {
            UInt64 max_memory = (UInt64)1024 * 1024 * 1024 * 30; // 30 Gigabyte
            IntPtr handle = CreateJobObject(IntPtr.Zero, null);
            var info = new JOBOBJECT_BASIC_LIMIT_INFORMATION
            {
                LimitFlags = 0x00000100 // JOB_OBJECT_LIMIT_PROCESS_MEMORY
            };
            var extendedInfo = new JOBOBJECT_EXTENDED_LIMIT_INFORMATION
            {
                BasicLimitInformation = info,
                ProcessMemoryLimit = max_memory,
                PeakProcessMemoryUsed = max_memory,
                JobMemoryLimit = max_memory,
                PeakJobMemoryUsed = max_memory
            };
